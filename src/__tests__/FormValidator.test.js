...
describe('Check e-mail', () => {
  test('A valid e-mail must return true', () => {
    expect(validateEmail('victor.jordan@gmail.com')).toBe(true);
  });
  test('A invalid e-mail must return false', () => {
    expect(validateEmail('victor.jorda')).toBe(false);
  });
});

describe('Check full name', () => {
  test('A full name with less than 8 characters must return false', () => {
    expect(validateFullname('victor')).toBe(false);
  });
  test('A full name with more than 8 characters must return true', () => {
